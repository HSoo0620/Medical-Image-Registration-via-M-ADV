...
class SwinAffine(nn.Module):
    def __init__(self, config):
        ...
        self.aff_head = nn.Linear(embed_num, 100)
        ...
        self.scl_head = nn.Linear(embed_num, 100)
        ...
        self.trans_head = nn.Linear(embed_num, 100)
        ...
        self.shear_head = nn.Linear(embed_num, 100)
        ...
        self.rigid_trans = RigidTransformer()
    ...
    def forward(self, x):
        ...
        # shr = torch.tanh(shr) * np.pi
        affine_out, affine_mat, inv_mat = self.affine_trans(source, aff, scl, trans, shr)
        #out, mat, inv_mat = self.affine_trans(source, aff)

        ###### Rigid 평가를 위함 ######
        scl = torch.clamp(scl, min=1.0, max=1.0)
        Rigid_out, Rigid_mat, Rigid_inv_mat = self.rigid_trans(source, aff, scl, trans)

        return affine_out, affine_mat, inv_mat, Rigid_out, Rigid_mat, Rigid_inv_mat
...
class RigidTransformer(nn.Module):
    ...
    def forward(self, src, affine, scale, translate):
        ...
        scale_y = scale[:, 1]
        scale_z = scale[:, 2]
        # scale_x = torch.tensor([2]).cuda()
        # scale_y = torch.tensor([2]).cuda()
        # scale_z = torch.tensor([2]).cuda()
        trans_x = translate[:, 0]
        trans_y = translate[:, 1]
        trans_z = translate[:, 2]
        ...
        trans = torch.stack([trans_x, trans_y, trans_z], dim=1).unsqueeze(dim=2)
        mat = torch.bmm(scale_mat, torch.bmm(rot_mat_z, torch.matmul(rot_mat_y, rot_mat_x)))
        inv_mat = torch.inverse(mat)
        mat = torch.cat([mat, trans], dim=-1)
        inv_trans = torch.bmm(-inv_mat, trans)
        inv_mat = torch.cat([inv_mat, inv_trans], dim=-1)
