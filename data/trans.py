...
class RandomRotion(Base):
    ...
    def tf(self, img, label):
        ...
        for bs in range(bsize):
            ...
            print(self.angle_buffer)
            label[bs,...] = rotate(label[bs,...], self.angle_buffer, axes=self.axes_buffer, reshape=False, order=0, mode='constant', cval=-1)
            print(self.angle_buffer)
        return img, label
